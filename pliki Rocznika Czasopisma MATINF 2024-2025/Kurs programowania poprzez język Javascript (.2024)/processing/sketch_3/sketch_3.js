...
function rysuj_spacer(spacer, labirynt, n, m, size) {
 
  for(var i = 0; i < spacer.length; ++i) {
     var k = spacer[i];
     circle(size*3/2 + k[0]*size, size*3/2 + k[1]*size, size/2);
  }
...
function spacer(labirynt, n, m, rozmiar_komnaty) {
  
  for(var i = 0; i < n; ++i) {
     for(var j = 0; j < m; ++j) {
        labirynt[i][j].odwiedzone = false; 
     }
  }
  
  var start = [0,0];
  labirynt[start[0]][start[1]].odwiedzone = true;
  var stos = [start];

  while(true) {

     var aktualna = stos[stos.length-1];
     var nastepna = 0;
     var stop = false;

      shuffleArray(labirynt[aktualna[0]][aktualna[1]].przejscia);
     for(var k = 0; k < labirynt[aktualna[0]][aktualna[1]].przejscia.length; ++k) {
       nastepna_k = labirynt[aktualna[0]][aktualna[1]].przejscia[k];
       if(labirynt[nastepna_k[0]][nastepna_k[1]].odwiedzone == false) {
          labirynt[nastepna_k[0]][nastepna_k[1]].odwiedzone = true;
          stos.push(nastepna_k);
          stop = true;
          break;
       }
     }
     ...
     if(!stop) {
       ...
       rysuj_spacer(stos, labirynt, n, m, rozmiar_komnaty);
       return;
     }
...
function setup() {
  ...
  console.log(pamiec);
  spacer(pamiec, n_lab_dlugosc, m_lab_wysokosc, rozmiar_komnaty);
}
