...
function rysuj_labirynt(pamiec, n, m, size, group) {
  ...
  function rysuj_sciane(s1,s2,t1,t2,size) {
	...
	const geometry_wall = new THREE.PlaneGeometry( 1, 1 );
	const material_wall = new THREE.MeshBasicMaterial( {color: 0xffff00, side: THREE.DoubleSide} );
	const plane = new THREE.Mesh( geometry_wall, material_wall );
	const wall_group = new THREE.Group();
	wall_group.translateX((s1+0.5)*size);
	wall_group.translateY((l+1)*size);
	wall_group.rotation.set(1.57,0,0);
	wall_group.scale.set(size, size, size);
	wall_group.add(plane);
	group.add(wall_group);
      ...
      if(s2 == t2) {
		...
		const plane = new THREE.Mesh( geometry_wall, material_wall );
		const wall_group = new THREE.Group();
		wall_group.translateX((k+1)*size);
		wall_group.translateY((s2+0.5)*size);
		wall_group.rotation.set(0,1.57,0);
		wall_group.scale.set(size, size, size);
		wall_group.add(plane);
		group.add(wall_group);
...
function animate() {
	...
	camera.position.x = Math.sin(3*clock.getElapsedTime());
