...
for(var i = 0; i < ilosc_gwiazd; ++i) {
	...
	gwiazdy.push({
		...
		"odleglosc_od" : function(gwiazda) {
			return Math.sqrt(Math.pow(this.x-gwiazda.x, 2) + Math.pow(this.y-gwiazda.y, 2) + Math.pow(this.z-gwiazda.z, 2));
		},
		"threejs_group" : null,
		"przetworz_grupe" : function() {
			//usun poprzednia grupe
			if(this.threejs_group != null) {
				this.usun_grupe();
			}
			//dodaj nowa grupe z nowym promieniem
			var group = new THREE.Group();
			group.position.x = this.x;
			group.position.y = this.y;
			group.position.z = this.z;
			gwiazdy[i].threejs_group = group;
			var geometry = new THREE.SphereGeometry( this.promien, 32, 32 );
		...
		"usun_grupe" : function() {
			scene.remove(this.threejs_group);
			this.threejs_group = null;
		}
	});
}
