...
class Mapa:
    ...
    def obiekty_w_sasiedztwie(self, robot, kierunek) :
        miejsce = self.gdzie_jest_obiekt(robot)
        if miejsce == None :
            return []
        x = miejsce[0]
        y = miejsce[1]
        if kierunek == 0 :
            if self.czy_poza_mapa(x, y - 1) :
                return []
            return self.pola_elementow[(x, y - 1)]
        if kierunek == 1 :
            if self.czy_poza_mapa(x + 1, y) :
                return []
            return self.pola_elementow[(x + 1, y)]
        if kierunek == 2 :
            if self.czy_poza_mapa(x, y + 1) :
                return []
            return self.pola_elementow[(x, y + 1)]
